...
public interface Pledgeable {
	...
	default void fromNbtPledgeable(NbtCompound nbt) {
		NbtList pledgedPlayerUUIDs = nbt.getList("PledgedPlayerUUIDs", NbtType.COMPOUND);
		for (int i = 0; i < pledgedPlayerUUIDs.size(); i++) {
			getPledgedPlayerUUIDs().add(pledgedPlayerUUIDs.getCompound(i).getUuid("UUID"));
		}
		setTimeSinceLastAttack(nbt.getInt("TimeSinceLastAttack"));
	}
	
	default void toNbtPledgeable(NbtCompound nbt) {
		...
		nbt.put("PledgedPlayerUUIDs", pledgedPlayerUUIDs);
		nbt.putInt("TimeSinceLastAttack", getTimeSinceLastAttack());
	}
}
