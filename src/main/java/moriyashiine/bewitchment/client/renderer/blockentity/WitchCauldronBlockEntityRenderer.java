...
public class WitchCauldronBlockEntityRenderer implements BlockEntityRenderer<WitchCauldronBlockEntity> {
	private static final List<RenderLayer> PORTAL_LAYERS = IntStream.range(1, 16).mapToObj((i) -> RenderLayer.getEndPortal()).collect(Collectors.toList());
	...
	private void renderName(WitchCauldronBlockEntity entity, BlockPos pos, MatrixStack matrices, VertexConsumerProvider vertexConsumers, int light) {
		if (entity.hasCustomName()) {
			if (pos.getSquaredDistance(MinecraftClient.getInstance().getBlockEntityRenderDispatcher().camera.getPos(), true) <= 256) {
				matrices.push();
	...
	private void renderPortal(WitchCauldronBlockEntity entity, BlockPos pos, MatrixStack matrices, VertexConsumerProvider vertexConsumers) {
		matrices.push();
		double distance = pos.getSquaredDistance(MinecraftClient.getInstance().getBlockEntityRenderDispatcher().camera.getPos(), true);
